		...
		<div>
		<label>Nom</label>
		<input class="inscription" name="nom" class="case" type="text" value="PAULHE">
		</div>
		<div>
		<label>Prénom</label> 
		<input class="inscription" name="prenom" type="text" value="Guillaume">
		</div>
		<div>
		<label>Age</label> 
	...
	if(array_key_exists('refuser', $_POST)) {
		...
		function edit($db,$id){
		$edit_req_ma_table = $db->prepare("UPDATE participants SET Nom = '$edit_nom', Prenom = '$edit_prenom', Age = $edit_age, Poids = $edit_poids, Taille = $edit_taille, Sexe = '$edit_sexe', Ceinture = '$edit_ceinture'  WHERE idParticipant = 9");
