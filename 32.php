		...
		<label>Prénom</label> 
		<input class="inscription" name="prenom" type="text" value="Rolland">
		<label>Nom</label>
		<input class="inscription" name="nom" class="case" type="text" value="Culet">
		<label>Age</label> 
		<input class="inscription" name="age" type="number" value="9">
		<label>Poids</label>
		<input class="inscription" name="poids" type="number" value="32">
		<label>Taille</label> 
		<input class="inscription" name="taille" type="number" value="117"> 
		<label>Sexe</label> 
		<?php
		if ($s == "Homme"){
	...
	if(array_key_exists('refuser', $_POST)) {
		...
		function edit($db,$id){
		$edit_req_ma_table = $db->prepare("UPDATE participants SET Nom = '$edit_nom', Prenom = '$edit_prenom', Age = $edit_age, Poids = $edit_poids, Taille = $edit_taille, Sexe = '$edit_sexe', Ceinture = '$edit_ceinture'  WHERE idParticipant = 32");
