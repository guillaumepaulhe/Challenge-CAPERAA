		...
		<div>
		<label>Prénom</label> 
		<input class="inscription" name="prenom" type="text" value="Gabriel">
		</div>
		<div>
		<label>Age</label> 
		...
		<div>
		<label>Poids</label>
		<input class="inscription" name="poids" type="number" value="70">
		</div>
		<div>
		<label>Taille</label> 
		<input class="inscription" name="taille" type="number" value="175"> 
		</div>
		<div>
		<label>Sexe</label> 
		<?php
	...
	if(array_key_exists('valider', $_POST)) {
		...
		function edit($db,$id){
		$edit_req_ma_table = $db->prepare("UPDATE participants SET Nom = '$edit_nom', Prenom = '$edit_prenom', Age = $edit_age, Poids = $edit_poids, Taille = $edit_taille, Sexe = '$edit_sexe', Ceinture = '$edit_ceinture'  WHERE idParticipant = 13");
